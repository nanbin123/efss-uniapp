	...
	export default {
		...
		methods: {
			...
			confirm(){
				...
				post("customer/insertCustomerProduct", {"productJson":JSON.stringify(productArray)}).then(res => {
					uni.hideLoading();
		...
		onReachBottom() {
			if (this.totalCount > this.productList.length) {
				this.pageNum++;
				post("product/selectListCustomerProduct", {
		...
		watch: {
			searchVal(val) {
				this.pageNum = 1;
				post("product/selectListCustomerProduct",{"pageNum": this.pageNum,"productNameOrType": val,"customerId":this.customerId
				}).then(res => {
					this.totalCount =  res.data.total				
/* 					let searchProducts = this.selectProductList;			
					let arrProducts = [];
					for (var i = 0; i <searchProducts.length ; i++) {
						let  searchProductId = searchProducts[i].id;
						let count =0;
						for (var j = 0; j < res.rows.length; j++) {
							let rowProductId = res.rows[j].id  ;
							if(rowProductId == searchProductId){
								res.rows[j].number = searchProducts[i].number;
								count ++;
								break;
							}
						}
						if(count ==0){
							arrProducts.push(searchProducts[i])
						}
					}
					this.productList = res.rows.concat(arrProducts); */		
		...
		onLoad() {
			...
			post("product/selectListCustomerProduct", {
				"pageNum": this.pageNum,"customerId":this.customerId}).then(res => {
				let that = this;				
